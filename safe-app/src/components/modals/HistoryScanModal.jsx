...
const HistoryScanModal = () => {
  ...
  const getRiskColor = (level, classification) => {
    if (classification) {
      return classification === 'SINUS' ? 'var(--danger)' : 'var(--success)';
    }
    switch(level) {
      case 'Low Risk':
            ...
            <div key={entry.id} className="timeline-item animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
              ...
              <div className="timeline-content card">
                <div className="timeline-card-header">
                  ...
                  <div className="risk-badge" style={{ backgroundColor: `${getRiskColor(entry.riskLevel, entry.classification)}20`, color: getRiskColor(entry.riskLevel, entry.classification) }}>
                    <Activity size={16} /> {entry.classification || entry.riskLevel}
                  </div>
                </div>
                ...
                <div className="scores-grid">
                  <div className="score-item">
                    ...
                    <div className="progress-container"><div className="progress-bar" style={{ width: entry.maxScore ? `${(entry.scores.symptoms / 150) * 100}%` : `${entry.scores.symptoms}%` }}></div></div>
                  </div>
                  <div className="score-item">
                    <span className="score-label">Lifestyle</span>
                    <div className="progress-container"><div className="progress-bar" style={{ width: entry.maxScore ? `${(entry.scores.lifestyle / 15) * 100}%` : `${entry.scores.lifestyle}%` }}></div></div>
                  </div>
                  <div className="score-item">
                    <span className="score-label">Environment</span>
                    <div className="progress-container"><div className="progress-bar" style={{ width: entry.maxScore ? `${(entry.scores.environmental / 28) * 100}%` : `${entry.scores.environmental}%` }}></div></div>
                  </div>
                  <div className="score-item">
                    <span className="score-label">Mucus Score</span>
                    {entry.scores.mucus !== undefined ? (
                      <div className="progress-container"><div className="progress-bar" style={{ width: entry.maxScore ? `${(entry.scores.mucus / 2) * 100}%` : `${entry.scores.mucus}%`, backgroundColor: 'var(--primary)' }}></div></div>
                    ) : (
                ...
                {expandedId === entry.id && (
                  <div className="details-section animate-fade-in" style={{ marginTop: '1rem', padding: '1rem', backgroundColor: 'var(--bg-secondary)', borderRadius: '8px', fontSize: '0.9rem' }}>
                    ...
                    {entry.maxScore && <p style={{ margin: '0.25rem 0', fontWeight: 'bold' }}>Final Score: {entry.finalScore} / {entry.maxScore}</p>}
                    <p style={{ margin: '0.25rem 0' }}><strong>Symptoms Score:</strong> {entry.maxScore ? `${entry.scores.symptoms} / 150` : `${entry.scores.symptoms}%`}</p>
                    <p style={{ margin: '0.25rem 0' }}><strong>Lifestyle Score:</strong> {entry.maxScore ? `${entry.scores.lifestyle} / 15` : `${entry.scores.lifestyle}%`}</p>
                    <p style={{ margin: '0.25rem 0' }}><strong>Environment Score:</strong> {entry.maxScore ? `${entry.scores.environmental} / 28` : `${entry.scores.environmental}%`}</p>
                    {entry.scores.mucus !== undefined ? (
                      <p style={{ margin: '0.25rem 0' }}><strong>Mucus Scan Score:</strong> {entry.maxScore ? `${entry.scores.mucus} / 2` : `${entry.scores.mucus}%`} {entry.mucusColorLabel ? `(Detected: ${entry.mucusColorLabel})` : ''}</p>
                    ) : (
